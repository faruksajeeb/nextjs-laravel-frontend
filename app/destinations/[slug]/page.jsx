...
const DESTINATIONS = [
  {
    ...
    excerpt: "Beaches, temples, rice terraces and island culture — for relaxation and adventure.",
    description:
...
const PACKAGES = [
  { id: "bali-7d", title: "7-Day Bali Escape", slug: "bali-7d", destination: "bali", price: 8990, duration: 7 },
  { id: "bali-4d", title: "4-Day Ubud & Beaches", slug: "bali-4d", destination: "bali", price: 4990, duration: 4 },
  { id: "kyoto-5d", title: "5-Day Kyoto Highlights", slug: "kyoto-5d", destination: "kyoto", price: 7990, duration: 5 },
];
...
export default function DestinationView({ params }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-black text-white">
        <div className="text-center">
          ...
          <Link href="/destinations" className="inline-block bg-indigo-600 text-white px-4 py-2 rounded-md">
  ...
  return (
    <>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="bg-white/5 backdrop-blur-md p-6 rounded-2xl shadow-lg">
              <div className="flex items-center justify-between">
                ...
                <Link href="/packages" className="text-sm text-indigo-300 hover:underline">All packages</Link>
              </div>

              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                {related.length ? (
                  related.map((p) => (
                    <article key={p.id} className="rounded-xl overflow-hidden bg-white/6 border border-white/6 p-4 flex items-start gap-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-300 mt-3">A curated experience focusing on the best of {dest.name}.</p>

                        <div className="mt-3 flex items-center gap-2">
                          <Link href={`/packages/${p.slug}`} className="px-3 py-2 text-sm rounded-lg bg-white/10 hover:bg-white/20 transition">
                            View Details
                          </Link>
                          <Link href={`/booking?pkg=${encodeURIComponent(p.slug)}`} className="px-3 py-2 text-sm rounded-lg bg-gradient-to-r from-indigo-500 to-pink-500 text-white">
                            Book
                          </Link>
                        </div>
                      </div>
                  ...
                  <div className="col-span-full text-gray-400">No packages yet for this destination.</div>
                )}
              </div>
            </section>
            ...
            <section className="bg-white/5 backdrop-blur-md p-6 rounded-2xl shadow-lg">
              ...
              <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
                {dest.gallery.map((g, i) => (
                  <button key={i} onClick={() => setSelectedImg(g)} className="overflow-hidden rounded-lg focus:outline-none">
                    <img src={g} alt={`${dest.name} ${i + 1}`} className="w-full h-28 object-cover transform hover:scale-105 transition" />
                  </button>
                ))}
              </div>
            </section>
            ...
            <section className="bg-white/4 p-6 rounded-2xl shadow-md text-gray-300">
              ...
              <div className="mt-3 w-full h-48 rounded-md bg-black/40 flex items-center justify-center text-sm text-gray-400">
                Map placeholder — add embed or map component
              </div>
            </section>
          </div>
          ...
          <aside className="space-y-4">
            <div className="bg-white/6 backdrop-blur-md border border-white/8 rounded-2xl p-5 shadow-lg sticky top-6">
              ...
              <div className="mt-3 flex gap-2">
                <Link href="/packages" className="flex-1 px-3 py-2 rounded-md bg-white/10 text-sm hover:bg-white/20 text-white">
                  All Packages
                </Link>
                <Link href={`/booking?pkg=${related[0]?.slug || ""}`} className="flex-1 px-3 py-2 rounded-md bg-gradient-to-r from-indigo-500 to-pink-500 text-white">
                  Quick Book
                </Link>
              </div>
      ...
      {/* Lightbox */}
      {selectedImg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4" onClick={() => setSelectedImg(null)}>
          <button aria-label="Close" className="absolute top-6 right-6 text-white text-2xl" onClick={() => setSelectedImg(null)}>✕</button>
          <img src={selectedImg} alt="Preview" className="max-h-[90vh] max-w-[90vw] rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </>
  );
